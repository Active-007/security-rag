...
if __name__ == '__main__':
    ...
    test_files = [f for f in os.listdir(test_dir) if f.endswith('.docx')] if os.path.exists(test_dir) else []
    if test_files:
        docx_loader = OCRDOCLoader(filepath=os.path.join(test_dir, test_files[0]))
        doc = docx_loader.load()
        print(doc)
        print(len(doc))
    else:
        print(f"未找到测试文档: {test_dir}")
