...
if __name__ == '__main__':
    ...
    import os
    # 使用安保业务板块的PPT进行测试（需自行放置PPT测试文件）
    test_dir = os.path.join(config.DATA_DIR, 'security_duty_data')
    test_files = [f for f in os.listdir(test_dir) if f.lower().endswith(('.ppt', '.pptx'))] if os.path.exists(test_dir) else []
    if test_files:
        ppt_loader = OCRPPTLoader(filepath=os.path.join(test_dir, test_files[0]))
        doc = ppt_loader.load()
        print(doc)
    else:
        print(f"未找到PPT测试文档: {test_dir}")