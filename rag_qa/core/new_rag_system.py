...
class RAGSystem:
    ...
    def retrieve_and_merge(self, query, source_filter=None, strategy=None):  # 新增 strategy 参数
        logger.info(f"retrieve_and_merge 开始处理查询: '{query}', 业务板块过滤: {source_filter}")
    ...
    def generate_answer(self, query, source_filter=None, history=None):
        ...
        logger.info(f"generate_answer 开始处理查询: '{query}', 业务板块过滤: {source_filter}")
        ...
        if history is not None and not isinstance(history, list):
            ...
            valid_history = []
            for h in history:
                if isinstance(h, dict) and "question" in h and "answer" in h:
                    valid_history.append(h)
                else:
                    logger.warning(f"跳过无效的历史条目: {h}")
        ...
        try:
            ...
            # 截断提示（若超长）— 保留头部指令和尾部问题，截断中间上下文
            if len(prompt_input) > self.max_prompt_length:
                logger.warning(f"提示长度 {len(prompt_input)} 超过 {self.max_prompt_length}，进行截断")
                # 保留尾部（包含用户问题和回答指令），截断上下文部分
                tail_marker = "**问题**:"
                tail_idx = prompt_input.rfind(tail_marker)
                if tail_idx > 0:
                    tail = prompt_input[tail_idx:]  # 保留 "**问题**: ..." 及之后
                    head = prompt_input[:tail_idx]   # 系统指令 + 上下文 + 历史
                    # 给尾部预留空间，截断头部
                    max_head = self.max_prompt_length - len(tail) - 20  # 预留安全余量
                    if max_head > 100:
                        prompt_input = head[:max_head] + "\n...(上下文已截断)...\n" + tail
                    else:
