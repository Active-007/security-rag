...
def main(query_mode=True, directory_path="data"):
    ...
    if not query_mode:
        ...
        print("\n欢迎使用安保集团企业知识库交互式查询系统！")
        print(f"支持的业务板块：{valid_sources}")
        ...
        while True:
            ...
            source_filter_input = input(f"请输入业务板块 ({'/'.join(valid_sources)}) (直接回车默认不过滤)：").strip()
            source_filter = None  # 默认不过滤
            if source_filter_input:
                if source_filter_input in valid_sources:
                    source_filter = source_filter_input
                    logger.info(f"用户选择了业务板块过滤: {source_filter}")
                else:
                    logger.warning(
                        f"无效的业务板块 '{source_filter_input}'，将不过滤"
                    )
                    print(f"提示：输入的业务板块 '{source_filter_input}' 无效，将不过滤。")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SecurityRAG System Main Entry Point")
