...
class IntegratedQASystem:
    ...
    def call_dashscope(self, prompt):
        ...
        try:
            ...
            self.logger.error(f"call_dashscope LLM调用失败: {e}")
            return f"call_dashscope 错误：LLM调用失败 - {e}"
    ...
    def query(self, query, source_filter=None, session_id=None):
        ...
        if answer:
            ...
            if session_id:
                self.update_session_history(session_id, query, "未找到答案")
            processing_time = time.time() - start_time
            self.logger.info(f"query 查询处理耗时 {processing_time:.2f}秒")
...
def main():
    ...
    print(f"支持的业务板块：{qa_system.config.VALID_SOURCES}")
    ...
    try:
        while True:
            ...
            if query.lower() == "exit":
                ...
                print("再见！")
                break
            source_filter = input(f"请输入业务板块 ({'/'.join(qa_system.config.VALID_SOURCES)}) (直接回车默认不过滤): ").strip()
            if source_filter and source_filter not in qa_system.config.VALID_SOURCES:
                logger.warning(f"main 无效的业务板块 '{source_filter}'，将不过滤")
                source_filter = None
