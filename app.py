...
# 创建应用实例
app = FastAPI(title="企业知识库智能问答系统API", description="安保集团制度文档智能问答系统")
...
GREETING_PATTERNS = [
    {
        ...
        "response": "你好！我是安保集团智能助手，专注于为员工解答安保执勤和武装押运相关问题，很高兴为你服务！"
    },
    {
        "pattern": r"^(你是谁|您是谁|你叫什么|你的名字|who are you)",
        "response": "我是安保集团智能助手，你的企业知识库问答系统，致力于提供安保执勤、武装押运等制度文档相关的解答！"
    },
    {
        "pattern": r"^(在吗|在不在|有人吗|在不)",
        "response": "我在！我是安保集团智能助手，随时为你解答制度相关问题！"
    },
    {
        "pattern": r"^(干嘛呢|你在干嘛|做什么)",
        "response": "我正在待命，随时为你解答安保执勤和武装押运相关的制度问题！有什么我可以帮你的？"
    }
]
...
# 根路径重定向到index.html（禁止缓存，确保每次加载最新版本）
@app.get("/")
async def read_root():
...
async def query(request: QueryRequest):
    ...
    if greeting_response:
        ...
        if request.session_id:
            try:
                qa_system.update_session_history(session_id, request.query, greeting_response)
            except Exception as e:
                qa_system.logger.error(f"问候语记录历史失败: {e}")
        return {
    ...
    if answer:
        ...
        if request.session_id:
            try:
                qa_system.update_session_history(session_id, request.query, answer)
            except Exception as e:
                qa_system.logger.error(f"BM25答案记录历史失败: {e}")
        ...
        while True:
            ...
            if greeting_response:
                ...
                try:
                    ...
                    qa_system.logger.error(f"WebSocket 问候语记录历史失败: {e}")
                if websocket.client_state == websocket.client_state.CONNECTED:
                    await websocket.send_json({
                        "type": "token",
